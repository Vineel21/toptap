...
class MediaPickerHelper {
  ...
  Future<XFile?> compressVideo(String filePath, String localPath) async {
    ...
    MediaInfo? file = await VideoCompress.compressVideo(
      filePath,
  ...
  Future<XFile?> compressVideoLowQuality(
      String filePath) async {
    MediaInfo oldFile = await getVideoInfo(filePath);
    Loggers.success(
        'Old File (low quality retry) ${oldFile.toJson()}');
    MediaInfo? file = await VideoCompress.compressVideo(
      filePath,
    ...
    Loggers.success(
        'Compress Video (LowQuality) : ${file?.toJson()}');
    return XFile(file?.path ?? '');
  }
