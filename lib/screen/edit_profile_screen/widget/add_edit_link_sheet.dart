...
class AddEditLinksSheet extends StatefulWidget {
  ...
  const AddEditLinksSheet(
      {super.key,
      this.link,
      required this.onLinksUpdate,
      required this.type});

  @override
  State<AddEditLinksSheet> createState() =>
      _AddEditLinksSheetState();
}

class _AddEditLinksSheetState
    extends State<AddEditLinksSheet> {
  ...
  void initState() {
    ...
    linkController =
        TextEditingController(text: widget.link?.url ?? '');
  }
  ...
  void _onSave() async {
    if (titleController.text.trim().isEmpty) {
      return baseController
          .showSnackBar(LKey.urlTitleEmpty.tr);
    }
    ...
    if (!AppRes.urlRegex
        .hasMatch(linkController.text.trim())) {
    ...
    switch (widget.type) {
      ...
      case LinkType.edit:
        Link link = (response.data ?? []).firstWhere(
            (element) => element.id == widget.link?.id);
