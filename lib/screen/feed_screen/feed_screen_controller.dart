...
class FeedScreenController extends BaseController {
  ...
  Rx<PostCategory> selectedPostCategory =
      PostCategory.discover.obs;
  ...
  Future<void> _fetchStory({bool isEmpty = false}) async {
    ...
    List<User> items =
        await PostService.instance.fetchStory();
    if (isEmpty) {
      stories.clear();
    }
  ...
  Future<void> fetchDiscoverPost(
      {bool isEmpty = false}) async {
    if (isLoading.value) return;
    isLoading.value = true;
    List<Post> _post = await PostService.instance
        .fetchPostsDiscover(type: PostType.posts);
    _addDataInPostList(_post, isEmpty);
  }

  Future<void> _fetchPostsFollowing(
      {bool isEmpty = false}) async {
    if (isLoading.value) return;
    isLoading.value = true;
    List<Post> _post = await PostService.instance
        .fetchPostsFollowing(type: PostType.posts);
    _addDataInPostList(_post, isEmpty);
  }

  Future<void> _fetchPostsNearBy(
      {bool isEmpty = false}) async {
    if (isLoading.value) return;
    isLoading.value = true;
  ...
  _addDataInPostList(
      List<Post> newList, bool isEmpty) async {
    if (isEmpty) {
      posts.clear();
    }
  ...
  void onWatchStory(
      List<User> users, int index, String watchType) {
    Get.bottomSheet(
      StoryViewSheet(
        stories: users,
        userIndex: index,
        onUpdateDeleteStory: (story) {
          ...
          if (userId ==
            ...
            final userIndex =
                stories.indexWhere((u) => u.id == userId);
            if (userIndex != -1) {
