...
class AudioDetailsScreenController extends BaseController {
  ...
  Future<void> _updateSavedMusicId(
      List<int> savedMusicIds) async {
    await UserService.instance
        .updateUserDetails(savedMusicIds: savedMusicIds);
  ...
  Future<void> fetchReelPostsByMusic() async {
    ...
    List<Post> items = await PostService.instance
        .fetchReelPostsByMusic(
            musicId: musicId,
            lastItemId: reelPosts.isEmpty
                ? null
                : reelPosts.last.id?.toInt());
    isLoading.value = false;

    reelPosts.addAll(items);
    music
        .update((val) => val?.postCount = reelPosts.length);
  }
  ...
  void onPlayPauseMusic() {
    ...
    DefaultCacheManager()
      ...
      isPlaying.value = true;
      await audioPlayerController.setFinishMode(
  ...
  void audioPlayerEventListener() {
    audioPlayerController.onPlayerStateChanged
        .listen((event) {
      switch (event) {
