...
class SearchScreenController extends BaseController {
  ...
  SearchScreenController({this.initialTabIndex});

  List<SearchTabs> searchTabs = SearchTabs.values;
  Rx<SearchTabs> selectedTabIndex =
      SearchTabs.values.first.obs;
  ...
  void onInit() {
    super.onInit();
    int startIndex = initialTabIndex ?? 0;
    if (startIndex != 0) {
      pageController =
          PageController(initialPage: startIndex);
    }
    onSearchTabTap(startIndex);
  }
  ...
  Future<void> searchUsers({bool reset = false}) async {
    ...
    List<User> items = await SearchService.instance
        .searchUsers(
  ...
  Future<void> searchHashTags({bool reset = false}) async {
    ...
    List<Hashtag> items = await SearchService.instance
        .searchHashtags(
            ...
            lastItemId:
                reset ? null : hashtags.lastOrNull?.id);
    if (reset) {
      hashtags.clear();
    }
    if (items.isNotEmpty) {
  ...
  void onLocationTap(Places place) {
    double latitude =
        place.location?.latitude?.toDouble() ?? 0.0;
    double longitude =
        place.location?.longitude?.toDouble() ?? 0.0;
    LatLng latLng = LatLng(latitude, longitude);
    Get.to(
        LocationScreen(
            latLng: latLng, placeTitle: place.title),
        preventDuplicates: false);
  }
}
