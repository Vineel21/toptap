...
class PostScreenController extends BaseController {
  ...
  PostScreenController(
      this.postData, this.isFromSinglePostScreen);
  ...
  Future<void> _likePostApi(Post? post) async {
    StatusModel model = await PostService.instance
        .likePost(postId: post?.id ?? -1);

    if (model.status == true) {
      if (post?.user?.notifyPostLike == 1 &&
  ...
  Future<void> _disLikePostApi(Post? post) async {
    await PostService.instance
        .disLikePost(postId: post?.id?.convertInt ?? -1);
  }
  ...
  void onComment(
    ...
    if (isFromSinglePostScreen) {
      if (Get.isRegistered<CommentSheetController>()) {
        final controller =
            Get.find<CommentSheetController>();
  ...
  Future<void> _savePostApi(Post? post) async {
    await PostService.instance
        .savePost(postId: post?.id?.convertInt ?? -1);
  }

  Future<void> _unSavePostApi(Post? post) async {
    await PostService.instance
        .unSavePost(postId: post?.id?.convertInt ?? -1);
  }
  ...
  void handleDelete(Post post,
      {required bool isModerator}) async {
    Get.bottomSheet(
      ConfirmationSheet(
          title: LKey.deletePostTitle.tr,
          onTap: () =>
              _deletePost(post, isModerator: isModerator),
  ...
  void _deletePost(Post post,
    ...
    if (isModerator) {
      model = await ModeratorService.instance
          .moderatorDeletePost(postId: post.id);
    } else {
  ...
  void handleReport(Post? post) {
    ...
    Get.bottomSheet(
        ReportSheet(
            id: post.id, reportType: ReportType.post),
        isScrollControlled: true);
  }
